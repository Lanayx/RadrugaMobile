...
//kendo widget
var kendo = kendo || { data: { binders: { source: { extend: function () { } } }, Binder: { extend: function () { } } } }, mobiscroll = mobiscroll || {}; mobiscroll.kendo = {};
(function (a) {
    mobiscroll.kendo.preventUpdate = {}; mobiscroll.kendo.getMobiBinder = function (p, d, i, h, x) {
        function q(d, g, c) { mobiscroll.kendo.preventUpdate[a(c).attr("id")] = !0; d.set(a(c).mobiscroll("getVal")); delete mobiscroll.kendo.preventUpdate[a(c).attr("id")] } function g(d, g) { mobiscroll.kendo.preventUpdate[a(d).attr("id")] || a(d).mobiscroll("setVal", g, !0, !1) } i || (i = g); h || (h = q); return {
            init: function (g, h, c) {
                kendo.data.Binder.fn.init.call(this, g, h, c); var c = a(g).data("mobiscroll-options"), i = {}; c && (i = (new Function("$root",
                "return " + c))(h[p].parents[0])); var c = a.extend(!0, "string" === typeof d ? { preset: d } : d, i, x || {}), q = this; a(q.element).on("change", function () { q.change(h, g) }).mobiscroll(c)
            }, refresh: function (a) { var d = this.bindings[p].get(); i(this.element, d, a ? a.action : void 0) }, change: function (a, d) { h(this.bindings[p], a, d) }
        }
    }
})(jQuery);
(function (a) {
    var p, d; d = navigator.userAgent.match(/Android|iPhone|iPad|iPod|Windows|Windows Phone|MSIE/i); if (/Android/i.test(d)) { if (p = "android-holo", d = navigator.userAgent.match(/Android\s+([\d\.]+)/i)) d = d[0].replace("Android ", ""), p = 4 <= d.split(".")[0] ? "android-holo" : "android" } else if (/iPhone/i.test(d) || /iPad/i.test(d) || /iPod/i.test(d)) { if (p = "ios", d = navigator.userAgent.match(/OS\s+([\d\_]+)/i)) d = d[0].replace(/_/g, ".").replace("OS ", ""), p = "7" <= d ? "ios" : "ios-classic" } else if (/Windows/i.test(d) || /MSIE/i.test(d) ||
    /Windows Phone/i.test(d)) p = "wp"; a.mobiscroll.themes[p] && (a.mobiscroll.defaults.theme = p)
})(jQuery);
(function() {
...
(function () {
     kendo.data.binders["mobiscroll-select"] = kendo.data.Binder.extend(mobiscroll.kendo.getMobiBinder("mobiscroll-select", "select"))
})();
