define(["require", "exports", "../geoLocation/GeoCoordinate", "../geoLocation/GeoDirection"], function (require, exports, coord, direction) {
    "use strict";
    function areCoordinatesEqual(coord1, coord2, accuracy) {
        return coord1.getDistance(coord2).then(function (dist) { return (dist < accuracy); });
    }
    exports.areCoordinatesEqual = areCoordinatesEqual;
    function getPathLength() {
        var coordinates = [];
        for (var _i = 0; _i < arguments.length; _i++) {
            coordinates[_i - 0] = arguments[_i];
        }
        return getGeolib().then(function (geoLib) {
    ...
    function getDistanceInDirection(start, end, direct) {
        ...
        switch (direct) {
            case direction.GeoDirection.East: {
                ...
                return start.getDistance(dirCoord).then(function (val) {
    ...
    exports.getDistanceInDirection = getDistanceInDirection;
    function getCoordinateByVector(start, bearing, distance) {
        var toDeg = function (angle) { return (angle * 180 / Math.PI); };
        var toRad = function (angle) { return (angle * Math.PI / 180); };
        var radius = 6371e3, x = distance / radius, y = toRad(bearing), a1 = toRad(start.latitude), b1 = toRad(start.longitude);
        ...
        b2 = (b2 + 3 * Math.PI) % (2 * Math.PI) - Math.PI;
        var result = new coord.GeoCoordinate();
        result.latitude = toDeg(a2);
        result.longitude = toDeg(b2);
        return result;
    }
    exports.getCoordinateByVector = getCoordinateByVector;
